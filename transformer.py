...
class SelfAttention(nn.Module):
  ...
  def forward(self, values, keys, query, mask):
    ...
    query = keys.reshape(N, query_len, self.heads, self.head_dim)
  ...
  class TransformerBlock(nn.Module):
    ...
    def forward(self, value, key, query, mask):
      attention = self.attention(value, key, query, maks)
...
class Tranformer(nn.Module):
  def __init__(
      ...
      device="cuda",
      max_length=100):
    super(Trasnformer, self).__init__()
  ...
  def make_src_mask(self, src):
    src_mask = (src != self.src.pad_idx).unsqueeze(1).unsqueeze(2)
    # (N, 1, 1, src_len)
